...
def load_data(path_to_data):
    ...
    points_world = np.loadtxt(os.path.join(path_to_data, 'p_W_corners.txt'))
    points_camera = np.loadtxt(os.path.join(path_to_data, 'detected_corners.txt'))
    # Homogenous coordinates and dimensions
    points_world = np.hstack([points_world, np.ones(points_world.shape[0], 1)])
    points_camera = points_camera.reshape((-1, 12, 2))
    points_camera = np.concatenate([points_camera, np.ones((points_camera.shape[0], 1, 1))], axis=2)
